    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group ">
                @can('portal.merchant-permission.create')
                    <a class="layui-btn layui-btn-sm" href="{{ route('admin.merchant-permission.create') }}">添 加</a>
                @endcan
            </div>
        </div>
        <div class="layui-card-body">
            ...
            <script type="text/html" id="options">
                <div class="layui-btn-group">
                    @can('portal.merchant-permission.edit')
                        <a class="layui-btn layui-btn-sm" lay-event="edit">编辑</a>
                    @endcan
                    @can('portal.merchant-permission.destroy')
...
@section('script')
    @can('portal.merchant-permission')
        <script>
            layui.config({
                ...
                var dataTable = function () {
                    treetable.render({
                        ...
                        elem: '#dataTable',
                        url: "{{ route('admin.merchant-permission.data') }}",
                        cols: [[ //表头
                ...
                table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if (layEvent === 'del') {
                        layer.confirm('确认删除吗？', function (index) {
                            layer.close(index)
                            var load = layer.load();
                            $.post("{{ route('admin.merchant-permission.destroy') }}", {
                                _method: 'delete',
                                ids: [data.id]
                            }, function (res) {
                                layer.close(load);
                        ...
                        location.href = '/admin/merchant-permission/' + data.id + '/edit';
                    }
                });
            })
        </script>
