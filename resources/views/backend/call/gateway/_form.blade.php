...
<div class="layui-form-item">
    ...
    <div class="layui-input-inline">
        <select name="merchant_id" lay-verify="required" @if(isset($model)) disabled @endif>
            <option value=""></option>
            @foreach($merchants as $d)
                <option value="{{$d->id}}" {{isset($model)&&$model->freeswitch_id==$d->id?'selected':''}} >{{$d->company_name}}</option>
            @endforeach
        </select>
    </div>
</div>
...
<div class="layui-form-item">
    <div class="layui-input-block">
        ...
        <a href="{{route('backend.call.gateway')}}" class="layui-btn layui-btn-sm" >返 回</a>
    </div>
</div>
