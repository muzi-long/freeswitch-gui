    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <form class="layui-form">
                <div class="layui-btn-group">
                    @can('frontend.crm.project-design.destroy')
                        <button class="layui-btn layui-btn-sm layui-btn-danger" type="button" id="listDelete">删 除</button>
                    @endcan
                    @can('frontend.crm.project-design.create')
                        <a class="layui-btn layui-btn-sm" href="{{ route('frontend.crm.project-design.create') }}">添 加</a>
                    @endcan
                </div>
            </form>
        </div>
        <div class="layui-card-body">
            ...
            <script type="text/html" id="options">
                <div class="layui-btn-group">
                    @can('frontend.crm.project-design.edit')
    ...
    <script>
        layui.use(['layer','table','form'],function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable'
                ,height: 500
                ,url: "{{ route('frontend.crm.project-design') }}" //数据接口
                ,page: true //开启分页
                ,cols: [[ //表头
            ...
            table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'del'){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("{{ route('frontend.crm.project-design.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                            if (result.code==0){
                    ...
                    location.href = '/frontend/crm/project-design/'+data.id+'/edit';
                }
            });
            ...
            $("#listDelete").click(function () {
                ...
                if (ids.length>0){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("{{ route('frontend.crm.project-design.destroy') }}",{_method:'delete',ids:ids},function (result) {
                            layer.close(index);
                            var icon = result.code==0?1:2;
