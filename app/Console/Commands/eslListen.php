...
class eslListen extends Command
{
    ...
    public function handle()
    {
        ...
        while (true) {
            //录音目录
            $filepath = $this->fs_dir . '/' .$this->recording_dir. '/' . date('Y/m/d/');
                ...
                switch ($eventname){
                    ...
                    case 'CHANNEL_HANGUP_COMPLETE':
                        if (Redis::hexists($this->hash_table,$uuid)){
                            ...
                            $cdr = DB::table($this->cdr_table)->where('uuid',$data['pid'])->whereNull('end_time')->first();
                            if ($cdr != null){
                                $callsec = $cdr->answer_time != null ? time()-strtotime($cdr->answer_time) : 0;
                                //更新通话时长
